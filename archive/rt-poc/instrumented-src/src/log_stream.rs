...
impl log::Log for Logger {
    ...
    fn log(&self, record: &Record) {
        ...
        // Passive timing of the `println!` below (POC 2b investigation, see
        // `workload::note_log_print`): `esp_println` holds a critical
        // section (`MIE = 0`) for the whole write *including* busy-waiting
        // on the 64-byte USB-Serial-JTAG FIFO, so a line longer than the
        // FIFO blocks every interrupt until the host drains it. Nothing
        // here changes what is printed. Gated because this logger can run
        // before `esp_hal::init`, when reading SYSTIMER isn't safe.
        let timed_from = crate::workload::log_timing_enabled().then(esp_hal::time::Instant::now);

        // Replaces esp_println's logger entirely -- this is the only
        // place these lines get printed locally, not a second sink next
        // to it.
        // `exp-quiet-print` counter-experiment (POC 2b): the periodic
        // heartbeat/logs lines are not printed locally (they still go to the
        // outbound log ring below, unchanged). Never true in the default build.
        let quiet = crate::workload::quiet_print_suppresses(record.target());
        if !quiet {
            esp_println::println!("{} - {}", record.level(), record.args());
        }

        let timed_to = timed_from.map(|_| esp_hal::time::Instant::now());

        let mut line: HString<LINE_MAX> = HString::new();
        let fits = write!(line, "{}", record.args()).is_ok();
        if let (Some(start), Some(end), false) = (timed_from, timed_to, quiet) {
            crate::workload::note_log_print(start, end, if fits { line.as_str() } else { "" });
        }
...
pub async fn run(stack: Stack<'static>, storage: &'static SharedStorage, tls: TlsReferenceStatic) -> ! {
    ...
    loop {
        // Passive timing marker for the POC 2b jitter investigation (see
        // `workload::ActivityGuard`).
        let activity = crate::workload::ActivityGuard::begin(crate::workload::Activity::Logs);
        let ctrl_url = agent::ctrl_url(storage).await;
        if let Some((host, port)) = split_host_port(&ctrl_url)
            && let Ok(host_c) = CString::new(host)
        {
            let result = run_session(tls, stack, storage, &mut rx_buffer, &mut tx_buffer, &host_c, port).await;
            crate::workload::note_logs_result(result.is_ok());
            if let Err(e) = result {
