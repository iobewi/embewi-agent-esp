...
async fn main(spawner: Spawner) -> ! {
    ...
    // POC 1+2: agent/workload native-artefact isolation spike (see
    // src/workload.rs's doc comment -- not wired to any real deployment
    // path). `map_and_run` itself is synchronous and one-shot (maps the
    // workload, calls `workload_entry()` once, arms a TIMG1 interrupt bound
    // to `workload_timer_isr`), returning control to the normal boot
    // sequence below before anything else spawns; `workload::monitor` is a
    // separate, ordinary task that just logs the workload's own interrupt
    // counter periodically. GPIO10 is claimed by `crates/workload-poc`
    // (both `workload_entry`'s sanity blink and `workload_timer_isr`'s
    // steady toggling), not by the agent -- if the status LED above *is*
    // configured on GPIO10, change that crate's pin before flashing to
    // avoid the two fighting over it. TIMG1 is likewise workload-only:
    // TIMG0 is already `esp_rtos::start()`'s scheduler tick, above.
    embewi_agent_esp::workload::map_and_run(peripherals.TIMG1);
    spawner.spawn(embewi_agent_esp::workload::monitor().unwrap());
