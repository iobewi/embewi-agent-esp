...
pub async fn run(stack: Stack<'static>, storage: &'static SharedStorage, tls: TlsReferenceStatic) -> ! {
    ...
    loop {
        ...
        // `workload::ActivityGuard`): when this iteration's work started and
        // ended, in RAM, no serial print.
        let activity = crate::workload::ActivityGuard::begin(crate::workload::Activity::Heartbeat);
        crate::workload::note_link(stack.is_link_up(), stack.is_config_up());
        ...
        if ctrl_url.is_empty() {
            ...
            {
                let ok = send(tls, stack, storage, &mut rx_buffer, &mut tx_buffer, &host_c, port).await;
                crate::workload::note_heartbeat_result(ok);
            }
        }
        drop(activity);
...
async fn send(
    ...
    let mut buf = [0u8; 64];
    let ok = match session.read(&mut buf).await {
        Ok(n) => {
        ...
        Err(e) => {
            warn!("heartbeat: reading response from {host_str} failed: {e}");
            false
        }
    };
